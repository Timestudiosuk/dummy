...
else if($.isPlainObject(value)||$.isArray(value)){return $ush.rawurlencode(JSON.stringify(value))}
return ''+value};$ush.toPlainObject=function(value){try{value=JSON.parse(unescape(''+value)||'{}')}catch(err){value={}}
...
return null}}(jQuery);var _window=window,_document=document,_undefined=undefined;_window.$ush=_window.$ush||{};_window.$usof=_window.$usof||{}
...
return!1},isValidState:function(state){return state&&(this._states||[]).indexOf(''+state)!==-1},isVCParamValue:function(){return this.$input.hasClass('wpb_vc_param_value')},toString:function(value){return $ush.toString(value)},toPlainObject:function(value){try{value=JSON.parse($ush.rawurldecode(''+value)||'{}')}catch(e){value={}}
return value},getDefaultValue:function(){var self=this;return!$ush.isUndefined(self._std)?self._std:''},getValueByState:function(state,value){var self=this;if(self.isResponsiveValue(value)){if(!self.isValidState(state)){state='default'}
...
var value=self.getValue();$usof.instance.valuesChanged[self.groupName]=value;self.$field.trigger('change',value)}},getDefaultValue:function(){var self=this;return!$ush.isUndefined(self._std)?self._std:''},setValue:function(value){var self=this;if(typeof value==='string'&&self.hasStringValue){try{value=JSON.parse($ush.rawurldecode(value)||'[]')}catch(err){console.error(value,err);value=[]}}
...
groupParams.fields[k].trigger('change');break}});self._reInitParams()},getValue:function(){var self=this,result=[];$.each(self.groupParams,function(i,groupParams){result.push(groupParams.getValues())});if(self.hasStringValue){try{result=$ush.toString(result)}catch(err){console.error(result,err);result=''}}
...
if(isRow&&(!isInGroup||self.isGroupParams)){self.fields[name]=$field.usofField(node)}else if(isGroup){self.groups[name]=$field.usofGroup(node)}});for(var fieldName in self.showIfDeps){if(!self.showIfDeps.hasOwnProperty(fieldName)||$ush.isUndefined(self.fields[fieldName])){continue}
...
return result},fireFieldEvent:function($container,trigger){if(!$container.hasClass('usof-form-row')){$('.usof-form-row',$container).each(function(_,row){var $row=$(row),isShown=$row.data('isShown');if(isShown===_undefined){isShown=$row.css('display')!='none'}
...
return!1};$.fn.usPopup=function(){this.each(function(){this.data('USOF_Popup',new $usof.popup(this.data('popup-id')))})}})(jQuery);!function($){"use strict";var US_Colpick={init:function(options){if(!!this.inited){return}
...
$usof.field.select={init:function(options){var self=this;self.parentInit(options);self.$container=$('.usof-select',self.$row);self.$hint=$('.usof-form-row-hint-text',self.$row);self.hintsJson={};var $hintsJson=$('.usof-form-row-hint-json',self.$row);if($hintsJson.length){self.hintsJson=$hintsJson[0].onclick()||{};$hintsJson.remove()}
self.$input.on('change',function(){self._changeSelect();self._toggleGridLayoutDesc()});self.$container.attr('selected-value',self.$input.val());self.$input.trigger('change')},_changeSelect:function(){var self=this,value=''+self.$input.val(),$selectedOption=self.$input.find(":selected"),selectedDataID=$selectedOption.data('id'),selectedDataTitle=$selectedOption.data('title');self.$container.attr('selected-value',value);if(selectedDataID&&(''+selectedDataID).match(/\d+/)){value=''+selectedDataID}
if(!self.hintsJson.no_posts){if(value.length&&value.match(/\d+/)){var hint='';if(self.hintsJson.hasOwnProperty('edit_url')){var regex=/(<a [^{]+)({{post_id}})([^{]+)({{hint}})([^>]+>)/,editTitle=self.hintsJson.edit;if(selectedDataTitle){editTitle=self.hintsJson.edit_specific+' '+selectedDataTitle}
hint=self.hintsJson.edit_url.replace(regex,'$1'+value+'$3'+editTitle+'$5')}
self.$hint.html(hint)}else{self.$hint.html('')}}},_toggleGridLayoutDesc:function(){var self=this;if(!self.$row.hasClass('for_grid_layouts')){return}
var value=self.getValue(),isVC=self.$row.hasClass('us_select_for_vc'),isNumericValue=$.isNumeric(value),$addDesc=$('.us-grid-layout-desc-add',isVC?self.$row.parent():self.$row),$editLink=$('.us-grid-layout-desc-edit',isVC?self.$row.parent():self.$row);if(isNumericValue){$('.edit-link',$editLink).attr('href',(self.$container.data('edit_link')||'').replace('%d',value))}
$addDesc[isNumericValue?'addClass':'removeClass']('hidden');$editLink[isNumericValue?'removeClass':'addClass']('hidden')},setValue:function(value,quiet){var self=this;if(!$('option[value="'+value+'"]',self.$input).length){value=$('option:first',self.$input).attr('value')||''}
self.parentSetValue(value,quiet);self.$input.trigger('change')}}}(jQuery);!function($,undefined){var _window=window,_document=document;if(_window.$usof===undefined){return}
...
return fontName+'|'+fontWeights.join(',')}}}(jQuery);!function($,undefined){var _window=window,_undefined=undefined;if(_window.$usof===_undefined){return}
var _KEYCODES_={ENTER:13,BACKSPACE:8};$usof.field.autocomplete={init:function(){var self=this;self.disableScrollLoad=!1;self._prefix='params:';self._typingDelay=0.5;var defaultSettings={ajax_query_args:{action:'unknown',_nonce:''},multiple:!1,sortable:!1,params_separator:','};self.$container=$('.usof-autocomplete',self.$row);self.$toggle=$('.usof-autocomplete-toggle',self.$container);self.$options=$('.usof-autocomplete-options',self.$container);self.$search=$('input[type="text"]',self.$options);self.$list=$('.usof-autocomplete-list',self.$container);self.$message=$('.usof-autocomplete-message',self.$container);self.$value=$('> .usof-autocomplete-value',self.$container);self._settings=$.extend(defaultSettings,self.$container[0].onclick()||{});self.items={};$('[data-value]',self.$list).each(function(_,item){var $item=$(item);self.items[$item.data('value')]=$item});if(!self._settings.multiple){self.$options.on('click','.usof-autocomplete-selected',function(){var isShow=self.$toggle.hasClass('show');self._events.toggleList.call(self,{type:isShow?'blur':'focus'});if(!isShow){if(!!self.pid){clearTimeout(self.pid)}
self.pid=setTimeout(function(){self.$search.focus();clearTimeout(self.pid)},0)}})}else{self.$options.on('click','.usof-autocomplete-selected-remove',self._events.remove.bind(self))}
self.$list.off().on('mousedown','[data-value]',self._events.selected.bind(self)).on('scroll',self._events.scroll.bind(self));self.$search.off().on('keyup',self._events.keyup.bind(self)).on('input',self._events.searchDelay.bind(self)).on('focus blur',self._events.toggleList.bind(self));self._initValues.call(self);self.$container.toggleClass('multiple',self._settings.multiple);if(self._settings.multiple&&self._settings.sortable){self.dragdrop=new $usof.dragDrop(self.$options,{itemSelector:'> .usof-autocomplete-selected'});self.dragdrop.on('dragend',self._events.dragdrop.dragend.bind(self))}},loaded:!1,_events:{dragdrop:{dragend:function(target,e){var value=[],items=$('> .usof-autocomplete-selected',target.$container).toArray()||[],field=$(target.$container).closest('.type_autocomplete').data('usofField');for(var k in items){if(items[k].hasAttribute('data-key')){value.push(items[k].getAttribute('data-key'))}}
value=(value.length?value.join(field._settings.params_separator):'');if(field instanceof $usof.field){field.setValue(value)}}},remove:function(e){e.preventDefault();var self=this,$target=$(e.currentTarget),$selected=$target.closest('.usof-autocomplete-selected'),key=$selected.data('key');self._removeValue.call(self,key);$('[data-value="'+key+'"]',self.$list).removeClass('selected');$selected.remove()},searchDelay:function(e){if(!e.currentTarget.value){return}
var self=this;if(!!self._typingTimer){clearTimeout(self._typingTimer)}
self._typingTimer=setTimeout(function(){self._events.search.call(self,e);clearTimeout(self._typingTimer)},1000*self._typingDelay)},search:function(e){var self=this,$input=$(e.currentTarget),value=($.trim($input.val()).toLowerCase()).replace(/\=|\"|\s/,''),$items=$('[data-value]',self.$list),$groups=$('[data-group]',self.$list),filter=function($items){$items.addClass('hidden').filter('[data-text^="'+value+'"], [data-text*="'+value+'"]').removeClass('hidden');$groups.each(function(){var $group=$(this);$group.toggleClass('hidden',!$('[data-value]:not(.hidden)',$group).length)})};if(!value||value.length<1){$items.removeClass('hidden');return}
filter.call(self,$items);self.disableScrollLoad=!1;self._ajax.call(self,function(items){filter.call(self,$('> *',self.$list));if(value&&!$('[data-value]:not(.hidden)',self.$list).length){self._showMessage.call(self,self._settings.no_results_found)}else{self._clearMessage.call(self);self.$toggle.addClass('show')}})},selected:function(e){var self=this,$target=$(e.currentTarget),selectedValue=$target.data('value')||'';if($target.hasClass('selected')&&self._settings.multiple){self._removeValue.call(self,selectedValue);$('[data-key="'+selectedValue+'"]',self.$options).remove();$target.removeClass('selected')}else if(self._addValue.call(self,selectedValue)){if(!self._settings.multiple){$('.usof-autocomplete-selected',self.$options).remove();$('[data-value]',self.$list).removeClass('selected')}
self.$toggle.removeClass('show');$target.addClass('selected');self.$search.val('').before(self._getSelectedTemplate.call(self,selectedValue))}},scroll:function(e){var self=this,$target=$(e.currentTarget);if(!self.disableScrollLoad&&!self.loaded&&($target.scrollTop()+$target.height())>=(e.currentTarget.scrollHeight-1)){self._ajax.call(self,function(items){if($.isEmptyObject(items)){self.disableScrollLoad=!0}})}},keyup:function(e){var self=this;if(e.keyCode===_KEYCODES_.ENTER){var search=$.trim(self.$search.val()),$selected=$('[data-text="'+search+'"]:visible:first',self.$list);if(!$selected.length){$selected=$('[data-value]:visible:first',self.$list)}
if($selected.length){$selected.trigger('click')}}
if(e.keyCode===_KEYCODES_.BACKSPACE){if(!$.trim(self.$search.val())){self._clearMessage.call(self);$('.hidden',self.$list).removeClass('hidden');self.$toggle.addClass('show')}}},toggleList:function(e){var self=this,isFocus=(e.type==='focus'),pid=$ush.timeout(function(){self.$toggle.toggleClass('show',isFocus);$ush.clearTimeout(pid)},(isFocus?0:200));if(!$.trim(self.$search.val())){$('[data-value].hidden, [data-group].hidden',self.$list).removeClass('hidden')}}},_ajax:function(callback){var self=this;if(self.loaded){return}
var query_args=self._settings.ajax_query_args;if((!query_args.hasOwnProperty('action')||query_args.action==='unknown')&&$.isFunction(callback)){return callback.call(self,{})}
var data=$.extend(query_args||{},{offset:$('[data-value]:visible',self.$list).length,search:$.trim(self.$search.val()),});if(self._offset&&self._offset===data.offset){return}
self._offset=data.offset;self.loaded=!0;self.$container.addClass('loaded');self._clearMessage.call(self);self._offset=data.offset;if(data.offset){data.offset+=1}
var insertItem=function($el,name,value){if(!self.items.hasOwnProperty(value)){var text=(name||'').replace(/\s/,'').toLowerCase(),$item=$('<div data-value="'+$ush.stripTags(value)+'" data-text="'+$ush.stripTags(text)+'" tabindex="3">'+name+'</div>');$el.append($item);self.items[value]=$item}};$.get(ajaxurl,data,function(res){self.loaded=!1;self.$container.removeClass('loaded');self._clearMessage.call(self);if(!res.success){self._showMessage.call(self,res.data.message);return}
$.each(res.data.items,function(value,name){if($.isPlainObject(name)){$.each(name,function(_value,_name){var $groupList=$('[data-group="'+value+'"]:first',self.$list);if(!$groupList.length){$groupList=$('<div class="usof-autocomplete-list-group" data-group="'+value+'"></div>');self.$list.append($groupList)}
insertItem.call(self,$groupList,_name,_value)})}else{insertItem.call(self,self.$list,name,value)}});if($.isFunction(callback)){callback.call(self,res.data.items)}
self.trigger('data.loaded',res.data.items)},'json')},_initValues:function(){var self=this,loadParams=[],initValues=(self.$value.val()||'').split(self._settings.params_separator)||[];$('.usof-autocomplete-selected',self.$options).remove();initValues.map(function(key){if(!key){return}
var $item=$('[data-value="'+key+'"]:first',self.$list).addClass('selected');if($item.length){self.$search.before(self._getSelectedTemplate.call(self,key))}else{loadParams.push(key)}});if(loadParams.length){self.$search.val(self._prefix+loadParams.join(self._settings.params_separator));self._ajax.call(self,function(items){$('[data-key]',self.$options).remove();$('.selected',self.$list).removeClass('selected');$(initValues).each(function(_,key){if(self.items.hasOwnProperty(key)&&self.items[key]instanceof $){self.items[key].addClass('selected');self.$search.before(self._getSelectedTemplate.call(self,key))}})});self.$search.val('')}},_showMessage:function(text){var self=this;self.$list.addClass('hidden');self.$message.text(text).removeClass('hidden')},_clearMessage:function(){var self=this;self.$list.removeClass('hidden');self.$message.addClass('hidden').text('')},_addValue:function(key){var self=this,isNotEnabled=!1,values=[],value=key;if(self._settings.multiple){values=(self.$value.val()||'').split(self._settings.params_separator);for(var k in values){if(values[k]===key){isNotEnabled=!0;break}}
if(!isNotEnabled){values.push(key);value=(values||[]).join(self._settings.params_separator).replace(/^\,/,'')}}
if(!isNotEnabled){self.$value.val(value);self.trigger('change',[value]);return!0}
return!1},_removeValue:function(key){var self=this,values=(self.$value.val()||'').toLowerCase().split(self._settings.params_separator),index=values.indexOf(''+key);if(index!==-1){delete values[index];values=values.filter(function(item){return item!==_undefined});self.$value.val(values.join(self._settings.params_separator))}
self.trigger('change',[self.getValue()])},_getSelectedTemplate:function(key){var $selected=$('[data-value="'+key+'"]:first',this.$list);if(!$selected.length){return ''}
return '<span class="usof-autocomplete-selected" data-key="'+key+'">\
			...
			</span>'},getValue:function(){var self=this;return(self.$value instanceof $)?self.$value.val():''},setValue:function(value,quiet){var self=this;self.$value.val(value);self._initValues.call(self);if(!quiet){self.trigger('change',[value])}}}}(jQuery);!function($,undefined){var _window=window,_undefined=undefined;if(_window.$usof===_undefined){return}
...
var self=this,title=(self.options.dynamicValues||{})[value]||value;self.$inputUrl.addClass('hidden').val('');self.$dynamicValue.data('value',value).removeClass('hidden').find('.usof-link-dynamic-value-title').attr('title',title).text(title)},removeDynamicValue:function(){var self=this;self.$dynamicValue.data('value',null).addClass('hidden');self.$inputUrl.removeClass('hidden')},_toggleUrlSearchResults:function(e){var self=this,isFocus=(e.type==='focus'),pid=$ush.timeout(function(){self.$urlSearchResults.toggleClass('hidden',!isFocus);$ush.clearTimeout(pid)},(isFocus?0:200))},_urlSearchResultsSelect:function(e){var self=this,$target=$(e.currentTarget),selectedUrl=$target.data('url')||'';if(selectedUrl){self.$inputUrl.val(selectedUrl);self._urlSearchResultsClear()}},_urlSearchResultsClear:function(){var self=this;$ush.timeout(function(){self.urlSearchAjaxQueriesNumber=0;self.$urlSearchResults.removeClass('loading');self.$urlSearchResults.addClass('hidden');self.$urlSearchResults.find('.usof-link-search-results-item').remove()},200)},_inputUrlKeyUpDown:function(e){if(!e.currentTarget.value){return}
var self=this;if(!!self._typingTimer){clearTimeout(self._typingTimer)}
self._typingTimer=setTimeout(function(){self._urlSearchInit.call(self,e);clearTimeout(self._typingTimer)},1000*self._typingDelay)},_urlSearchInit:function(e){var self=this,$input=$(e.currentTarget),value=$.trim($input.val()).toLowerCase(),isUrl=!0;self.$urlSearchResults.find('.usof-link-search-results-item').remove();self.$urlSearchNotice.addClass('hidden');if(value===''||$.inArray(value.substring(0,1),['#','/','.'])!==-1){self._urlSearchResultsClear();return}
...
self.$urlSearchResults.show();self.$urlSearchResults.find('.usof-link-search-results-item').remove();self._urlSearchAjaxCall()},_urlSearchResultsScroll:function(e){var self=this,$target=$(e.currentTarget);if(!self.urlSearchParams.terms_search_completed&&!self.urlSearchDoingAjax&&($target.scrollTop()+$target.height())>=(e.currentTarget.scrollHeight-1)){self._urlSearchAjaxCall()}},_urlSearchAjaxCall:function(){var self=this,data=$.extend({},self.urlSearcAjaxQueryArgs||{},self.urlSearchParams);self.$urlSearchResults.addClass('loading');self.urlSearchDoingAjax=!0;self.urlSearchAjaxQueriesNumber++;var currentAjaxQueryNumber=self.urlSearchAjaxQueriesNumber;$.post(ajaxurl,data,function(res){if(self.urlSearchAjaxQueriesNumber==currentAjaxQueryNumber){if(!res.success){self.$urlSearchResults.find('.usof-link-search-results-item').remove();self.$urlSearchNotice.html(res.data.message).removeClass('hidden')}else if(res.data.items&&res.data.items.length>0){$.each(res.data.items,function(index,item){if($.isPlainObject(item)){var $item=$('<div class="usof-link-search-results-item" data-url="'+item.permalink+'">'+item.title+' <i>'+item.type+'</i></div>');self.$urlSearchResults.append($item)}})}else if(res.data.posts_offset==0&&res.data.terms_offset==0&&res.data.notice){self.$urlSearchResults.find('.usof-link-search-results-item').remove();self.$urlSearchNotice.html(res.data.notice).removeClass('hidden')}
self.urlSearchParams.posts_offset=res.data.posts_offset;self.urlSearchParams.posts_search_completed=res.data.posts_search_completed;self.urlSearchParams.terms_offset=res.data.terms_offset;self.urlSearchParams.terms_search_completed=res.data.terms_search_completed;self.$urlSearchResults.removeClass('loading');self.urlSearchDoingAjax=!1;self.urlSearchAjaxQueriesNumber=0}},'json')},updateValue:function(){var self=this,result={};var dynamicValue=self.$dynamicValue.data('value');if(dynamicValue){dynamicValue=(''+dynamicValue).split('|');result.type=$ush.toString(dynamicValue[0]);if(dynamicValue.length==2){result[result.type]=$ush.toString(dynamicValue[1])}}
...
if(!$ush.isUndefined(data.url)){self.$inputUrl.val(data.url)}
else if(data.type){if(!$ush.isUndefined(data[data.type])){data.type+='|'+data[data.type]}
self.setDynamicValue(data.type)}
var setCheckboxes=!1;$.each(self.$checkboxes,function(_,checkbox){var name=checkbox.name,isChecked=!!data[name];$(checkbox).prop('checked',isChecked);setCheckboxes=setCheckboxes||isChecked});$('.action_toggle_menu',self.$row).toggleClass('has_values',setCheckboxes);$.each(self.relatedFields,function(name,$value){if(!$ush.isUndefined(data[name])){$value.val(data[name])}});self.updateValue()}
...
resultValue[responsiveState][param]=value}}.bind(this))}.bind(this));resultValue=$ush.toString(resultValue);if(resultValue===$ush.toString({})){resultValue=''}
this.$input.val(resultValue);if(!this._lastResultValue||this._lastResultValue!==resultValue){this._lastResultValue=resultValue;this.trigger('change',resultValue)}
