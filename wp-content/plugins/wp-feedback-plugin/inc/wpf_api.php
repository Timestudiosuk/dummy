...
function wpf_website_details(){
    ...
    //     $val=get_option("wpf_from_email_mode");
    //    if($val != FALSE){
    //         array_push($settings,['name' => 'wpf_from_email_mode', 'value' => $val]);
    //     }
 ...
 */
 function wpf_api_func_get_users()
 {
    ...
    foreach ( $wpfb_users as $user ) {
        ...
        $response[$wpf_temp_count]['wpf_display_name']  = htmlspecialchars($user->user_login, ENT_QUOTES, 'UTF-8');
    ...
    // echo json_encode($wpfb_users); die;
...
function wpf_api_func_get_tasks() {
	...
	foreach ($wpfb_tasks as $wpfb_task) {
	    ...
	    //$task_img = !empty(get_post_meta($wpfb_task->ID, 'wpf_task_screenshot', true)) ? 'data:image/jpg;base64,'.base64_encode(file_get_contents(get_post_meta($wpfb_task->ID, 'wpf_task_screenshot', true))) : '';
 ...
 * @return NULL
 */
// function wpf_crm_add_page_post( $post_id ) {
...
//     if ( $new_status != $old_status && in_array(get_post_type($post_id),$wpf_post_types) ) {
//         $response = array();
//         $response['wpf_site_id']=get_option('wpf_site_id');
//         $response['id']=$post_id;
//         $response['name']=htmlspecialchars(get_the_title($post_id), ENT_QUOTES, 'UTF-8');
//         $response['type']=get_post_type($post_id);
//         $response = json_encode($response);
//         wpf_send_remote_post($url,$response);
//     }
// }
...
function wpf_autologin($request){
   ...
   /* global $site_url;
    $actual_link = (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? "https" : "http") . "://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]";
    $site_url = $request['PHP_SELF'];
	if (!is_null($request['REDIRECT_URL']))
	{
		$site_url = $request['REDIRECT_URL'];
	}
	else if(((strpos($actual_link, 'wp-admin'))&&(strpos($actual_link, 'post_type=page'))) !== false){
	...
	if($_GET['wpf_taskid']) {
		$comment_id = $_GET['wpf_taskid'];
	}
	else if($_GET['wpf_general_taskid']){
		$comment_id = $_GET['wpf_general_taskid'];
	}
	else{
		$comment_id='';
	}
...
//    $res_body = json_decode($res['body']);
        if($res['status'] == 1){
            ...
            if ( !is_wp_error( $user ) )
            {
                ...
                //===old code===
            /* if (!is_null($posttype)){
                    $redirect_to = $site_url.'?'.$posttype.'&wpf_taskid='.$_GET['wpf_taskid'];
                }
                else if($_GET['wpf_taskid']){
                ...
                if($comment_id!=''){
                    ...
                    $task_info = $wpdb->get_results($sql, OBJECT);
                    ...
                    if($_GET['wpf_taskid']) {
                        ...
                        //$sql = "select post_id from wp_postmeta where meta_key='task_comment_id' and meta_value='".$comment_id."'";
                        ...
                        //echo $task_url = get_post_meta($task_id,'task_page_url',true);
...
//    $response['image']='site_image';
//    $response['favicon']='site_favicon';
    $response['from_plugin']=1;
    $response['users'] = json_decode(wpf_api_func_get_users());
    // $response['pages'] = json_decode(wpf_get_page_list('api'));

//    $settings = [];
//    $settings[] = ['name' => 'wpf_site_id', 'value' => $wpf_site_id];
//    $settings[] = ['name' => 'enabled_wpfeedback', 'value' => 'yes'];
...
//	update_user_meta(get_current_user_id(), 'wpf_user_type','advisor' );
	//clear_session();
    ...
    foreach ($settings->white_label as $key=>$val){
        if($key == 'wpfeedback_powered_by'){
            ...
            if($key=='wpfeedback_favicon'){
                $key='wpf_favicon';
                /*$attach_id = wpf_upload_icon_from_dashboard($val);
                $val = $attach_id;*/
            }
...
function wpf_send_remote_post($url,$response,$wpf_token='', $is_print = 0){
    ...
    if ( is_wp_error( $response ) ) {
        ...
        // if( !empty($real_response['data']['code']) && $real_response['data']['code'] == 403 ){
...
function move_all_old_data() {
    ...
    // array_push($settings, ['name' => 'wpf_from_email_mode', 'value' => get_option('wpf_from_email_mode') == "yes" ? 'yes' : "no"]);
    ...
    // 	$response_array2 = [];
// 	$response_array2['wpf_site_id'] = get_option('wpf_site_id');
// 	$response_array2['tasks'] = json_decode(wpf_api_func_get_tasks());
// 	echo "<pr."
// 	$url2 = WPF_CRM_API . 'move-site-old-tasks';
//     $body2 = json_encode($response_array2);
//     $res2 = wpf_send_remote_post($url2, $body2, "", 1);
