...
jQuery_WPF(document).ready(function(){
    ...
    //append div to load upgrade popup on front side by Pratap
    jQuery_WPF('body').append('<div id="hidden-upgrade-popup-box" style="z-index:999"></div>'); 
    jQuery_WPF('#hidden-upgrade-popup-box').load("wp-content/plugins/wp-feedback-plugin/inc/admin/upgrade-subscription-popup.php");
    ...
    // removing all wpf elements if the page loads in iframe
    // if ( window.location !== window.parent.location ) {
        
    //     jQuery_WPF("#wpf_panel").remove();
    //     jQuery_WPF(".wpfb-point").remove();
        
    //     return false
    // }
    /*if(wpf_tab_permission_user == undefined) {
        var wpf_tab_permission_user = 'yes';
    }
    if(wpf_tab_permission_priority == undefined) {
        var wpf_tab_permission_priority = 'yes';
    }
    if(wpf_tab_permission_status == undefined) {
        var wpf_tab_permission_status = 'yes';
    }
    if(wpf_tab_permission_screenshot == undefined) {
        var wpf_tab_permission_screenshot = 'yes';
    }
    if(wpf_tab_permission_information == undefined) {
        var wpf_tab_permission_information = 'yes';
    }
    if(wpf_tab_permission_delete_task == undefined) {
        var wpf_tab_permission_delete_task = 'yes';
    }
    if(wpf_tab_permission_auto_screenshot == undefined) {
        var wpf_tab_permission_auto_screenshot = 'yes';
    }
    if(wpf_tab_permission_display_stickers == undefined) {
        var wpf_tab_permission_display_stickers = 'yes';
    }
    if(wpf_tab_permission_display_task_id == undefined) {
        var wpf_tab_permission_display_task_id = 'yes';
    }*/
    ...
    wpf_mark_internal_btn='<i class="gg-chevron-double-left"></i><span class="wpf_tooltiptext unmark_internal_tooltip_text">Switch to a normal task</span><span class="wpf_tooltiptext new_internal_tooltip_text">Add comment and create Internal task</span><span class="wpf_tooltiptext mark_internal_tooltip_text">Switch to an Internal task</span>';
    /*jQuery_WPF(document).on('click', '.wpfbpriority', function(){
        var elemid = jQuery_WPF(this).attr('data-elemid');
        set_task_prioirty(elemid);
    });*/

    
    /*jQuery_WPF(document).on('click', '.wpfbtaskstatus', function(){
        var elemid = jQuery_WPF(this).attr('data-elemid');
    ...
    jQuery_WPF(document).on('mouseenter', '.popover', function(e) {
        ...
        // if ( typeof jQuery_WPF('#' + jQuery_WPF(this).prop('id')).find('.nav-tabs').prop('id') !== 'undefined' ) {
        //     bubble_id = jQuery_WPF('#' + jQuery_WPF(this).prop('id')).find('.nav-tabs').prop('id').split('myTab-');
...
function enable_comment(){
    ...
    jQuery(document).mousemove(function(e){
        ...
        if ( jQuery('#cursor_html') && !jQuery('.wpf_comment_container').hasClass('show') ) {
            jQuery('#cursor_html').text("Click to leave a comment");
        } else {
        ...
        html2canvas(document.body,{
            ...
            jQuery_WPF.ajax({
                ...
                success: function(data){
                    ...
                    var comment_html = '<li class="wpf_author is_image"><level class="task-author">'+current_user_name+'</level><div class="meassage_area_main"><a href="'+data+'" download>'+img_dwn_icon+'</a><a href="'+data+'" target="_blank"><img src="'+data+'" /></a></div></li>';
...
jQuery_WPF(window).on('load', function(){
    ...
    jQuery_WPF('#wpf_comment_mode_general_task svg').on('click',function(){
        /* => v2.1.0 */
        // jQuery_WPF(this).parent().trigger('click');
        ...
        if(this.id!="disable_comment_a"){
            if(comments==true){
                ...
                jQuery_WPF(this).parents().addBack().not('html').each(function() {
                    ...
                    // console.table(rightArrowParents );
...
function set_task_prioirty(id){
    ...
    jQuery_WPF.ajax({
        ...
        success : function(data){
            ...
            if(data != '') {
                jQuery_WPF(current_popup).find("#task_comments_"+id).append('<li class="wpf_other  is_info" data-comment_id=""><level class="task-author"><span>1 second ago</span></level><div class="meassage_area_main"><p class="chat_text">'+data+'</p></div></li>');
            }
...
function set_task_status(id,status=''){
    ...
    jQuery_WPF.ajax({
        ...
        success : function(data){
            ...
            if(data != '') {
                // console.log(data);
                jQuery("#task_comments_"+id).append('<li class="wpf_other  is_info" data-comment_id=""><level class="task-author"><span>1 second ago</span></level><div class="meassage_area_main"><p class="chat_text">'+data+'</p></div></li>');
            }
...
function wpf_delete_comment(comment_id){
    ...
    jQuery_WPF.ajax({
        ...
        success : function(data){
            ...
            // jQuery_WPF('.wpf_loader_'+id).hide();
            //jQuery_WPF(current_popup).find('.wpf_loader_'+id).hide();
            // jQuery_WPF("#wpf-chat-text-"+comment_id).text(current_user_name+'deleted this comment');
...
function new_comment(id, internal=0){
            ...
            if(jQuery_WPF('#wpf_mark_internal_'+id).hasClass('wpf_is_internal')){
               ...
               jQuery_WPF.ajax({
                    ...
                    console.log('hey' + data);
                        if(data == 'false'){
                        	if(jQuery_WPF('.wpf-uf-otheruser-notice').length > 0) {
                                jQuery_WPF('.wpf-uf-otheruser-notice').remove();
                            }
                            jQuery_WPF('.wpf-uf-popup-image img').attr('src', 'https://wpfeedback-image.s3.us-east-2.amazonaws.com/images/Internal+tasks.png');
                            jQuery_WPF('.wpf-uf-plan-title').text('Internal Tasks');
                            jQuery_WPF('.wpf-uf-plan-detail').html('Make it easy to collaborate with your team on tasks that your clients cannot see with internal tasks. Change the logo, icon and main color to give your clients’ a unique experience. <br>Available on <b>Starter</b>, <b>Expert</b> and <b>Agency</b>');
                            jQuery_WPF('.wpf-uf-plan-link').attr('href', 'https://atarim.io/upgrade/plan-1/');
    ...
    // if(jQuery_WPF('#popover-content-c' + id).length == 0 || jQuery_WPF('#popover-content-c' + id).length == ''){
        ...
        jQuery_WPF.ajax({
            ...
            success : function(data){
                ...
                if(internal=='1'){
                    ...
                    console.log(jQuery_WPF(notify_user_block).find('span').html());
        ...
        success : function(data){
            ...
            if(data!=0){
                ...
                if(new_task['task_type']=='general'){
                    ...
                    var view_id = bubble_comment_count-1;
                }
               ...
               var wpf_dal_taskmeta = jQuery_WPF('#wpf_display_all_taskmeta').is(":checked");
                ...
                img_dwn_icon = "";
                var comment_html = '<li class="wpf_author"><level class="task-author">'+current_user_name+' '+wpf_just_now+'</level><div class="meassage_area_main">'+img_dwn_icon+'<p class="chat_text">'+temp_task_text+'</p></div></li>';
                jQuery_WPF('#task_comments_'+id).append(comment_html);

                if(id == 1 && wpf_current_role == 'advisor'){
                    var masic_comment_html = "";
                    var masic_msg = "Awesome! you have just added your first task on the website. Let's start managing it from the dashboard. <a class='wpf_demo_cta' href='https://app.atarim.io/login' target='_blank'>Go to the Agency dashboard!!</a>";
                    var masic_current_user_name = "Atarim";
                    var masic_comment_html= '<li class="wpf_other magic_msg_replied"><level class="task-author">'+masic_current_user_name+' '+wpf_just_now+'</level><div class="meassage_area_main">'+img_dwn_icon+'<p class="chat_text">'+masic_msg+'</p></div></li>';
...
function load_wpfb_tasks(){
    ...
    if(current_page_id!=''){
	    ...
	    /*beforeSend: function(){
		jQuery_WPF('.wpf_page_loader').show();
	    },*/
            success: function(data){
                // console.log(data);
		...
		//console.log("comments", onload_wpfb_tasks);
                if(onload_wpfb_tasks !== '' && onload_wpfb_tasks !== null && onload_wpfb_tasks !== undefined){
                    ...
                    //     console.log(onload_wpfb_tasks['milestone']);
                    //     const milestone = onload_wpfb_tasks['milestone'];
                    //     const html      = `${milestone_clock_icon}
                    //         <div class="milestone_content">
                    //             <p>${milestone['title']}</p>
                    //             <h4 id="countdown_timer">${milestone['countdown_text']}</h4>
                    //         </div>`;
                    //     jQuery_WPF('#site_milestone').html(html);
...
// 						var uncompleted_task = total_task - (open_per + in_progress_per + pending_review_per);
                        
                        // show the individual task item on the sidebar
...
function generate_wpfb_task_html(wpfb_task_id,wpfb_metas){
    ...
    // get the WP user name (first and last name / display name)
    // wpfb_metas.task_config_author_name = get_wp_author_name( wpfb_metas );

    if(wpfb_metas.task_type=='general' || wpfb_metas.task_type=='email'){
        
        // console.log(wpfb_metas.task_config_author_name);
        ...
        for (var key in wpfb_metas.comments) {
            ...
            /* for (var k in wpfb_users_arr) {
                if (wpfb_users_arr.hasOwnProperty(k)) {
                    if(wpfb_users_arr[k]['username']==author || wpfb_users_arr[k]['username'] == wpfb_metas.comments[key].author){
                        author = wpfb_users_arr[k]['displayname'];
                        wpfb_metas.comments[key].author = wpfb_users_arr[k]['displayname'];
                        break;
                    }
                }
                console.log("author: "+ author+"; wpfbmetaauthor: "+ wpfb_metas.comments[key].author+"; wpfbuserarr: "+wpfb_users_arr[k]['displayname']);
             } */
            ...
            if(wpfb_metas.comments[key].comment_type==0){
                let  img_dwn_icon = "";
                wpfb_messages_html += '<li class="'+task_author+'" data-comment_id="'+key+'"><level class="task-author">'+author+'<span>'+wpfb_metas.comments[key].time+'</span>'+edited_html+'</level><div class="meassage_area_main"><p id="wpf-chat-text-'+key+'" class="chat_text">'+URLify(wpfb_metas.comments[key].message)+'</p>'+edit_delete_button_html+'</div><div id="wpfb-edit-comment-wrapper-'+key+'" class="wpfb-edit-comment-wrapper"><textarea class="form-control wpfb-edit-comment" data-comment_id="'+key+'" placeholder="Edit the comment..." spellcheck="false">'+URLify(wpfb_metas.comments[key].message)+'</textarea><button class="wpf_edit_comment_btn" onclick="wpfb_edit_comment('+key+')">'+edit_comment_text+'</button><a class="wpf-cancel-edit-comment" onclick="wpfb_cancel_edit_comment('+key+')" href="javascript:void(0)">'+cancel_edit_comment_text+'</a></div></li>';
            }
            else if (wpfb_metas.comments[key].comment_type==3) {
                let img_dwn_icon = "";
                wpfb_messages_html += '<li class="'+task_author+'" data-comment_id="'+key+'"><level class="task-author">'+author+'<span>'+wpfb_metas.comments[key].time+'</span></level><div class="meassage_area_main">'+img_dwn_icon+'<iframe width="100%" height="120" src="https://www.youtube.com/embed/'+wpfb_metas.comments[key].message+'"></iframe></div><div id="wpfb-edit-comment-wrapper-'+key+'" class="wpfb-edit-comment-wrapper"><textarea class="form-control wpfb-edit-comment" data-comment_id="'+key+'" placeholder="Edit the comment..." spellcheck="false">'+wpfb_metas.comments[key].message+'</textarea><button class="wpf_edit_comment_btn" onclick="wpfb_edit_comment('+key+')">'+edit_comment_text+'</button><a class="wpf-cancel-edit-comment" onclick="wpfb_cancel_edit_comment('+key+')" href="javascript:void(0)">'+cancel_edit_comment_text+'</a></div></li>';
            }
                ...
                // wpfb_messages_html += '<li class="'+task_author+' is_image"><level class="task-author">'+author+'<span>'+wpfb_metas.comments[key].time+'</span></level><div class="meassage_area_main"><span class="wpf_image_open"><a href="'+wpfb_metas.comments[key].message+'" target="_blank">'+image_external_icon+'</a></span>'+img_dwn_icon+'<img src="'+wpfb_metas.comments[key].message+'" class="wpf_task_uploaded_image" /></div></li>';
            }
            else{
       ...
       for (var key in wpfb_users_arr) {
        ...
        // console.log(jQuery_WPF(document).find(wpfb_metas.wpfb_task_bubble));
        let search_buble;
		try {
...
function wpf_generate_general_task_html(wpfb_task_id,wpfb_metas,load_general=0){
    ...
    for (var key in wpfb_metas.comments) {
        ...
        if(wpfb_metas.comments[key].comment_type==0){
            let img_dwn_icon = "";
            wpfb_messages_html += '<li class="'+task_author+'" data-comment_id="'+key+'"><level class="task-author">'+author+'<span>'+wpfb_metas.comments[key].time+'</span>'+edited_html+'</level><div class="meassage_area_main">'+img_dwn_icon+'<p id="wpf-chat-text-'+key+'" class="chat_text">'+URLify(wpfb_metas.comments[key].message)+'</p>'+edit_delete_button_html+'</div><div id="wpfb-edit-comment-wrapper-'+key+'" class="wpfb-edit-comment-wrapper"><textarea class="form-control wpfb-edit-comment" data-comment_id="'+key+'" placeholder="Edit the comment..." spellcheck="false">'+URLify(wpfb_metas.comments[key].message)+'</textarea><button class="wpf_edit_comment_btn" onclick="wpfb_edit_comment('+key+')">'+edit_comment_text+'</button><a class="wpf-cancel-edit-comment" onclick="wpfb_cancel_edit_comment('+key+')" href="javascript:void(0)">'+cancel_edit_comment_text+'</a></div></li>';
        }
        else if (wpfb_metas.comments[key].comment_type==1) {
            wpfb_messages_html += '<li class="'+task_author+' is_image" data-comment_id="'+key+'"><level class="task-author">'+author+'<span>'+wpfb_metas.comments[key].time+'</span></level><div class="meassage_area_main">'+img_dwn_icon+'<a href="'+wpfb_metas.comments[key].message+'" target="_blank"><img src="'+wpfb_metas.comments[key].message+'" class="wpf_task_uploaded_image" title="WPF Screenshot" alt="WPF Screenshot"/></a></div></li>';
        }
        else if (wpfb_metas.comments[key].comment_type==3) {
            let img_dwn_icon = "";
            wpfb_messages_html += '<li class="'+task_author+'" data-comment_id="'+key+'"><level class="task-author">'+author+'<span>'+wpfb_metas.comments[key].time+'</span></level><div class="meassage_area_main">'+img_dwn_icon+'<iframe width="100%" height="120" src="https://www.youtube.com/embed/'+wpfb_metas.comments[key].message+'"></iframe></div></li>';
        }
        else{
            let img_dwn_icon = "";
            var wpf_download_file  = wpfb_metas.comments[key].message.split("/").pop();
            wpfb_messages_html += '<li class="'+task_author+' is_file" data-comment_id="'+key+'"><level class="task-author">'+wpfb_metas.comments[key].author+'<span>'+wpfb_metas.comments[key].time+'</span></level><div class="meassage_area_main">'+img_dwn_icon+'<a href="'+wpfb_metas.comments[key].message+'" download="'+wpf_download_file+'" target="_blank"><i class="gg-software-download"></i> '+ wpf_download_file+'</a></div></li>';
        }
        jQuery_WPF('#task_comments_'+comment_count).append(wpfb_messages_html);
        console.log('#task_comments_' + wpfb_metas.task_comment_id);
    }
    ...
    /*const $last = jQuery('#task_comments_52 > .wpf_author:last-child');
    console.log(jQuery('#task_comments_52 > li').length);
    jQuery_WPF('#task_comments_52').animate({scrollTop: (jQuery('#task_comments_52 > li').length * 100)}, 2000);*/
...
function wpf_task_popover_html(wpf_task_type, comment_count, wpfb_task_id, wpfb_metas, wpfb_users_html, wpfb_task_priority_html, wpfb_task_status_html, wpfb_messages_html,new_task_popover,load_general) {
    ...
    // console.log("Current User: "+ wpf_config_author_name + "; Metas Task Author: "+ wpfb_metas.task_config_author_name);
    ...
    mark_as_complete_checkbox = "<input type='checkbox' data-id='"+comment_count+"' class='mark_as_complete_checkbox wpf_checkbox' id='mark_complete_"+comment_count+"' "+dissabled_attr+" " + task_status_complete_checked + "><label class='mark_as_complete_lable wpf_checkbox_label' for='mark_complete_"+comment_count+"'>Mark As Complete</label>";
    ...
    var comment_html = '<div class="form-group"><textarea class="form-control" id="comment-' + comment_count + '" placeholder="' + wpf_comment_box_placeholder + '"></textarea><span id="wpf_error_' + comment_count + '" class="wpf_hide">' + wpf_task_text_error_msg + '</span><span id="wpf_task_error_' + comment_count + '" class="wpf_hide">' + wpf_task_upload_error_msg + '</span><span id="wpf_push_to_media_error_' + comment_count + '" class="wpf_hide">' + wpf_push_to_media_error_msg + '</span><div class="bottom_buttons">'+mark_as_complete_checkbox+internal_button+'<button class="wpf_green_btn wpf_comment_btn" onclick="new_comment(' + comment_count + ')">' + wpf_add_comment_btn + '</button></div><button class="wpf_upload_button"><input type="file" name="wpf_uploadfile_' + comment_count + '" id="wpf_uploadfile_' + comment_count + '" data-elemid="' + comment_count + '" class="wpf_uploadfile"> <i class="gg-attachment"></i></button><p id="wpf_upload_error_' + comment_count + '" class="wpf_hide">' + wpf_upload_invalid_file_msg + '</p></div></div></div>';
...
function load_popover_content(element){
    // console.log("load_popover_content");
    // console.log(jQuery_WPF(element));
    if(current_page_id!=''){
        jQuery_WPF.ajax({
            method:"POST",
            url: ajaxurl,
            data: {action:'load_wpfb_tasks',wpf_nonce:wpf_nonce,current_page_url:current_page_url,fallback_link_check:fallback_link_check,current_page_id:current_page_id,page_type:page_type},
            beforeSend: function(){
                // load the loader only for currently opened popover => v2.1.0
                // if there is any other loader is running, current popover's loader will not show,
                // until the previous get hidden => v2.1.1
                // if ( jQuery_WPF('.wpf_loader').is(':hidden') ) {
...
// 						var uncompleted_task = total_task - (open_per + in_progress_per + pending_review_per);
                        ...
                        // console.log(value);
                        //generate_wpfb_task_html(value,onload_wpfb_tasks[value]);
                        // console.log(onload_wpfb_tasks[value].wpf_task_id, current_bubble);
                        // console.log(jQuery_WPF("#bubble-" + onload_wpfb_tasks[value].wpf_task_id).data('comments-available'));
...
function generate_popover_html(wpfb_task_id,wpfb_metas){
    ...
    if(wpfb_metas.task_type=='general'){
        ...
        for (var key in wpfb_metas.comments) {
            ...
            if(wpfb_metas.comments[key].comment_type==0){
                let  img_dwn_icon = "";
                wpfb_messages_html += '<li class="'+task_author+'" data-comment_id="'+key+'"><level class="task-author">'+author+'<span>'+wpfb_metas.comments[key].time+'</span>' + edit_delete_button_html + '</level><div class="meassage_area_main"><p id="wpf-chat-text-'+key+'" class="chat_text">'+URLify(wpfb_metas.comments[key].message)+'</p>'+edited_html+'</div><div id="wpfb-edit-comment-wrapper-'+key+'" class="wpfb-edit-comment-wrapper"><textarea class="form-control wpfb-edit-comment" data-comment_id="'+key+'" placeholder="Edit the comment..." spellcheck="false">'+URLify(wpfb_metas.comments[key].message)+'</textarea><button class="wpf_edit_comment_btn" onclick="wpfb_edit_comment('+key+')">'+edit_comment_text+'</button><a class="wpf-cancel-edit-comment" onclick="wpfb_cancel_edit_comment('+key+')" href="javascript:void(0)">'+cancel_edit_comment_text+'</a></div></li>';
            }
            else if (wpfb_metas.comments[key].comment_type==3) {
                let img_dwn_icon = "";
                wpfb_messages_html += '<li class="'+task_author+'" data-comment_id="'+key+'"><level class="task-author">'+author+'<span>'+wpfb_metas.comments[key].time+'</span></level><div class="meassage_area_main">'+img_dwn_icon+'<iframe width="100%" height="120" src="https://www.youtube.com/embed/'+wpfb_metas.comments[key].message+'"></iframe></div></li>';
       ...
       // wpf_bubble_tracker(comment_count,wpfb_metas.task_clean_dom_elem_path);
        ...
        // jQuery_WPF('body').scrollTop(jQuery_WPF('ul.wpf_current_chat_box li').last().position().top + jQuery_WPF('ul.wpf_current_chat_box li').last().height());
    }
...
function close_popover(comment_count){
    ...
    jQuery_WPF('#bubble-'+comment_count).trigger('click', [ "closed" ]);
}
        ...
        success: function(data){
            ...
            var wpfb_users_arr = JSON.parse(wpfb_users);
                ...
                jQuery_WPF.each(k,function (index, value) {
                    ...
                    // get the WP user name (first and last name / display name)
                    // onload_wpfb_tasks[value].task_config_author_name = get_wp_author_name( onload_wpfb_tasks[value] );
                    ...
                    jQuery_WPF("#wpf_allpages_container"+sidebar_class).html(wpfb_all_page_task_list_html);
...
function wpf_upload_file(obj){
    ...
    if(wpf_taskid > 0){
        jQuery_WPF.ajax({
            ...
            processData: false,
            beforeSend: function(){
                // console.log('.wpf_loader_'+elemid);
...
jQuery_WPF(document).ready(function(){
    ...
    jQuery_WPF('#wpf_sidebar_filter_task_status input[name="wpf_filter_task_status"]').click(function(){
        ...
        jQuery_WPF.each(jQuery_WPF('#wpf_sidebar_filter_task_status input[name="wpf_filter_task_status"]:checked'), function(){
            ...
            // show the header
            /*let first_visible_task = null;
            jQuery_WPF('.wpf_container.wpf_active_filter > ul > li').each(function(index, element) {

                // console.log(jQuery_WPF(element).css('display'));

                if ( jQuery_WPF(element).css('display') === 'flex' ) {

                    if ( jQuery_WPF(element).index() > 0 ) {

                        const cls = jQuery_WPF(element).parent().prop('id').split('_container_');

                        jQuery_WPF(element).addClass('header_'  + cls[1]);
                    }
                }
            });*/
        });
    ...
    jQuery_WPF('#wpf_sidebar_filter_task_priority input[name="wpf_filter_task_priority"]').click(function(){
        ...
        jQuery_WPF.each(jQuery_WPF('#wpf_sidebar_filter_task_priority input[name="wpf_filter_task_priority"]:checked'), function(){
            ...
            // if ( jQuery_WPF('.wpf_container.wpf_active_filter ul li > .wpf_task_number').hasClass(jQuery_WPF(this). val()) ) {
            //     jQuery_WPF('.wpf_container.wpf_active_filter ul li').show();
            // }
        });
...
function wpf_add_tag(id) {
    ...
    if(id !='' && tag_name !=''){
        jQuery_WPF.ajax({
            ...
            beforeSend: function(){
                // console.log('.wpf_loader_'+commentid);
