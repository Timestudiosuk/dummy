...
jQuery_WPF(document).ready(function () {
   ...
   /*STEP 1 BUTTON*/
    ...
    //remove notice from popup based on user by Pratap
    if(logged_user.current_user == 'advisor') {
        if(jQuery_WPF('.wpf-uf-otheruser-notice').length > 0) {
            jQuery_WPF('.wpf-uf-otheruser-notice').remove();
        }
    } else {
        if(jQuery_WPF('.wpf-uf-webmaster-notice').length > 0) {
            jQuery_WPF('.wpf-uf-webmaster-notice').remove();
        }
        if(jQuery_WPF('.wpf-uf-plan-link').length > 0) {
            jQuery_WPF('.wpf-uf-plan-link').remove();
        }
    }
    ...
    jQuery_WPF('#wpf_tasks div, #wpf_tasks button, #wpf_tasks input, #wpf_tasks a, #wpf_tasks select').on('click change keyup keydown keypress', function(e){
        if (jQuery_WPF(this).parents('.blocked').length) {
            jQuery_WPF('.wpf-uf-popup-image img').attr('src', 'https://wpfeedback-image.s3.us-east-2.amazonaws.com/images/Task+Center+-+Plugin.png');
            jQuery_WPF('.wpf-uf-plan-title').text('WP Admin Task Center');
            jQuery_WPF('.wpf-uf-plan-detail').html('Use the internal task centre to easily manage tasks from this website by unlocking filtering, bulk actions and access to task comment feeds. <br>Available on <b>Starter</b>, <b>Expert</b> and <b>Agency</b>');
            jQuery_WPF('.wpf-uf-plan-link').attr('href', 'https://atarim.io/upgrade/plan-2/');
    ...
    jQuery_WPF('#wpf_enabled_compact_mode').change(function(){
        if(jQuery_WPF(this).hasClass('blocked')) {
            jQuery_WPF('.wpf-uf-popup-image img').attr('src', 'https://wpfeedback-image.s3.us-east-2.amazonaws.com/images/Bottom+Bar.png');
            jQuery_WPF('.wpf-uf-plan-title').text('Bottom bar');
            jQuery_WPF('.wpf-uf-plan-detail').html('Approve pages entire pages with auto complete on all open tasks and have more control over your tasks with the bottom bar Available on <b>Starter</b>, <b>Expert</b> and <b>Agency</b>');
    ...
    jQuery_WPF('.wpf-whitelabel-parent.blocked input, .wpf-whitelabel-parent.blocked textarea, .wpf-whitelabel-parent.blocked button').on('click change keyup keydown keypress', function(){
        jQuery_WPF('.wpf-uf-popup-image img').attr('src', 'https://wpfeedback-image.s3.us-east-2.amazonaws.com/images/White+labelling.png');
        jQuery_WPF('.wpf-uf-plan-title').text('White Labelling');
        jQuery_WPF('.wpf-uf-plan-detail').html('Completely customise the look of Atarim on this website with white label settings. Change the logo, icon and main color to give your clients’ a unique experience. <br>Available on <b>Starter</b>, <b>Expert</b> and <b>Agency</b>');
        jQuery_WPF('.wpf-uf-plan-link').attr('href', 'https://atarim.io/upgrade/plan-2/');
        jQuery_WPF('.wpf-uf-pop-wrapper').show();
        if (this.checked == false) {
            jQuery_WPF(this).prop('checked', true);
    ...
    jQuery_WPF('.auto-report.blocked').change(function(){
        jQuery_WPF('.wpf-uf-popup-image img').attr('src', 'https://wpfeedback-image.s3.us-east-2.amazonaws.com/images/Autoreports.png');
        jQuery_WPF('.wpf-uf-plan-title').text('Auto Reports');
        jQuery_WPF('.wpf-uf-plan-detail').html('Automatically send 24 hour and 7 day reports to your clients with the click of a button, that summarise all activity on tasks, keeping your clients up to date on the work that’s been done. <br>Available on <b>Starter</b>, <b>Expert</b> and <b>Agency</b>');
        jQuery_WPF('.wpf-uf-plan-link').attr('href', 'https://atarim.io/upgrade/plan-1/');
        jQuery_WPF('.wpf-uf-pop-wrapper').show();
        if (this.checked == false) {
            jQuery_WPF(this).prop('checked', true);
    ...
    jQuery_WPF('.blocked .wpf_perm_table input[type=checkbox]').change(function(){
        jQuery_WPF('.wpf-uf-popup-image img').attr('src', 'https://wpfeedback-image.s3.us-east-2.amazonaws.com/images/User+Permissions.png');
        jQuery_WPF('.wpf-uf-plan-title').text('User Permissions');
        jQuery_WPF('.wpf-uf-plan-detail').html('Take full control over what users can do with Atarim user permissions and fully customise experience based on user role. <br>Available on <b>Starter</b>, <b>Expert</b> and <b>Agency</b>');
        jQuery_WPF('.wpf-uf-plan-link').attr('href', 'https://atarim.io/upgrade/plan-1/');
        jQuery_WPF('.wpf-uf-pop-wrapper').show();
        if (this.checked == false) {
            jQuery_WPF(this).prop('checked', true);
    ...
    jQuery_WPF('.wpf_global_settings').change(function () {
        ...
        if(jQuery_WPF(this).hasClass('blocked')) {
            jQuery_WPF('.wpf-uf-popup-image img').attr('src', 'https://wpfeedback-image.s3.us-east-2.amazonaws.com/images/Global+Settings+-+Plugin.png');
            jQuery_WPF('.wpf-uf-plan-title').text('Global Settings');
            jQuery_WPF('.wpf-uf-plan-detail').html('Save time and make it easy to set up this website by applying settings from your Agency Dashboard with one click, including white labelling, notification settings and permissions. <br>Available on <b>Starter</b>, <b>Expert</b> and <b>Agency</b>');
            jQuery_WPF('.wpf-uf-plan-link').attr('href', 'https://atarim.io/upgrade/plan-1/');
            jQuery_WPF('.wpf-uf-pop-wrapper').show();
            if(this.checked) {
    ...
    // toggle from email mode
    // jQuery_WPF(document).on('click', '#wpf_from_email_mode', function(e) {

    //     if ( jQuery_WPF(this).is(':checked') ) {

    //         // hide the textbox
    //         jQuery_WPF("#wpf_notifications_mode #wpf_from_email").hide();

    //         // hide the description
    //         jQuery_WPF("#wpf_notifications_mode > #unsafe").hide();
    //         jQuery_WPF("#wpf_notifications_mode > #safe").show();
    //     } else {

    //         // hide the textbox
    //         jQuery_WPF("#wpf_notifications_mode #wpf_from_email").show();

    //         // hide the description
    //         jQuery_WPF("#wpf_notifications_mode > #unsafe").show();
    //         jQuery_WPF("#wpf_notifications_mode > #safe").hide();
    //     }
    // });
});
...
function wpf_initial_setup_done(site_url, wpf_user) {
    ...
    jQuery_WPF.ajax({
        ...
        success: function (data) {
            ...
            console.log("In initial Done");
        }
    });
}

//function wpf_save_settings_btn() {
//    
//    var form_data = jQuery_WPF('#wpf_form_site_setting').serialize();
//    var wpf_file = jQuery_WPF('#wpf_logo_file')[0].files[0];
//    var wpf_upload_form = new FormData();
//    wpf_upload_form.append('action', 'site_settings_ajax');
//    wpf_upload_form.append("wpf_nonce", wpf_nonce);
//    wpf_upload_form.append("wpf_logo_file", wpf_file);
//    wpf_upload_form.append("form_data", form_data);
//    
//    jQuery_WPF.ajax({
//	type: 'POST',
//        url: ajaxurl,
...
//            jQuery_WPF('.wpf_loader_admin').hide();
//	    if(data.status == 0) {
//		jQuery_WPF('#setting_err_msg').html(data.message);
//		jQuery_WPF('#setting_err_msg').show();
//	    } else if(data.status == 1) {
//		jQuery_WPF('#setting_succ_msg').html(data.message);
//		jQuery_WPF('#setting_succ_msg').show();
//	    }
//        }
//    });
//}

//function wpf_save_permission_btn() {
//    
//    var form_data = jQuery_WPF('#wpf_form_site_permission').serialize();
//    jQuery_WPF.ajax({
//	type: 'POST',
//        url:ajaxurl,
//        method: 'POST',
//        data:{action: 'site_permission_ajax',wpf_nonce:wpf_nonce, 'form_data': form_data},
//	beforeSend: function(){
//	    jQuery_WPF('.wpf_loader_admin').show();
//	},
//        success: function (data) {
        ...
        jQuery_WPF.ajax({
            ...
            success: function (data) {
                ...
                if(response.status==1){
                    ...
                    if(response.type==1){
                        comment_html = '<li class="chat_author" id=""><level class="wpf-author">ME just now</level><p class="task_text"><a href="'+response.message+'" target="_blank"></a><div class="tag_img" style="width: 275px;height: 183px;"><div class="meassage_area_main"><a href="'+response.message+'" target="_blank"></a><img style="width: 100%;" class="wpfb_task_screenshot" src="'+response.message+'" />'+img_dwn_icon+'</div></div></p></li>';
                    }
                    else{
                        var wpf_download_file  = response.message.split("/").pop();
                        // var comment_html = '<li class="chat_author"><level class="wpf-author">ME just now</level><p class="task_text"><a href="'+response.message+'" download="'+wpf_download_file+'" target="_blank"><div class="meassage_area_main"><i class="gg-software-download"></i> '+ wpf_download_file+'</div></a></p></li>';
                        ...
                        comment_html = '<li class="chat_author"><level class="wpf-author">ME just now</level><p class="task_text">'+response_message+'</p></li>';

                        // comment_html=URLify(comment_html);
                    }
...
function wpf_generate_front_task(is_internal = 0){
