...
jQuery_WPF(window).on('load', function(){
    if(disable_for_admin == 0){
        ...
        jQuery_WPF( function() {
            ...
            // jQuery_WPF( ".wpf_launch_buttons_wrapper" ).draggable({ containment: "body" });
        });
    }
    ...
    jQuery_WPF.ajax({
        ...
        success : function(data){
            ...
            if(internal=='1'){
                ...
                console.log(jQuery_WPF(tagged_area_container).find(".wpf_task_type").length);
...
function generate_comment(id){
    ...
    jQuery_WPF.ajax({
        ...
        success: function(data){
            ...
            // var comment_html = '<li class="wpf_author"><level class="task-author">'+current_user_name+'<span>'+wpf_just_now+'</span></level><div class="meassage_area_main"><p class="chat_text">'+URLify(temp_task_text)+'</p></div></li>';
            jQuery_WPF('#task_comments_'+id).append(comment_html);
            jQuery_WPF('#comment-'+id).val('');
            /*jQuery_WPF('.wpf_current_chat_box').animate({scrollTop: jQuery_WPF(".wpf_current_chat_box li").last().offset().top},2000);*/
            jQuery_WPF('#task_comments_'+id).animate({scrollTop: jQuery_WPF('#task_comments_'+id).prop("scrollHeight")}, 2000);
    ...
    for (var usr in wpfb_users_arr) {
        if (wpfb_users_arr.hasOwnProperty(usr)) {
            if(wpfb_users_arr[usr]['username']==wpfb_metas.task_config_author_name){
                // wpfb_metas.task_config_author_name = wpfb_users_arr[usr]['first_name']
                // ? wpfb_users_arr[usr]['first_name'] + ' ' + wpfb_users_arr[usr]['last_name']
                // : wpfb_users_arr[usr]['displayname'];
    ...
    if(wpfb_metas.task_type=='general'){
        ...
        for (var key in wpfb_metas.comments) {
            ...
            if(wpfb_metas.comments[key].comment_type==0){
                ...
                img_dwn_icon  = "<span id='wpf_push_media' class='wpf_push_media wpf_image_download'>"+push_to_media_icon+"</span><span id='wpf_image_download' class='wpf_image_download' onclick='wpf_image_download_action(this)'>"+image_download_icon+"</span><span id='' class='wpf_image_open' onclick='wpf_image_open_new_tab(this)'>"+image_open_icon+"</span><span class='wpf_image_delete' onclick='wpf_task_image_delete(this)'>"+image_close_icon+"</span>";
                // wpfb_messages_html += '<li class="'+task_author+' is_image" data-comment_id="'+key+'"><level class="task-author">'+author+'<span>'+wpfb_metas.comments[key].time+'</span></level><div class="meassage_area_main">'+img_dwn_icon+'<img src="'+wpfb_metas.comments[key].message+'" class="wpf_task_uploaded_image" /></div></li>';
...
function wpf_generate_general_task_html(wpfb_task_id,wpfb_metas){
    ...
    for (var key in wpfb_metas.comments) {
        ...
        if(wpfb_metas.comments[key].comment_type==0){
            ...
            // img_dwn_icon  = "<span id='wpf_image_download' class='wpf_image_download' onclick='wpf_image_download_action(this)'>"+image_download_icon+"</span><span id='' class='wpf_image_open' onclick='wpf_image_open_new_tab(this)'>"+image_open_icon+"</span><span class='wpf_image_delete'>"+image_close_icon+"</span>";
            img_dwn_icon  = "<span id='wpf_push_media' class='wpf_push_media wpf_image_download'>"+push_to_media_icon+"</span><span id='wpf_image_download' class='wpf_image_download' onclick='wpf_image_download_action(this)'>"+image_download_icon+"</span><span id='' class='wpf_image_open' onclick='wpf_image_open_new_tab(this)'>"+image_open_icon+"</span><span class='wpf_image_delete' onclick='wpf_task_image_delete(this)'>"+image_close_icon+"</span>";
            // wpfb_messages_html += '<li class="'+task_author+' is_image" data-comment_id="'+key+'"><level class="task-author">'+author+'<span>'+wpfb_metas.comments[key].time+'</span></level><div class="meassage_area_main"><img src="'+wpfb_metas.comments[key].message+'" class="wpf_task_uploaded_image" /></div></li>';
            wpfb_messages_html += '<li class="'+task_author+' is_image" data-comment_id="'+key+'"><level class="task-author">'+author+'<span>'+wpfb_metas.comments[key].time+'</span></level><div class="meassage_area_main"><a href="'+wpfb_metas.comments[key].message+'" target="_blank">'+'</a>'+img_dwn_icon+'<img src="'+wpfb_metas.comments[key].message+'" class="wpf_task_uploaded_image" /></div></li>';
        }
        else if (wpfb_metas.comments[key].comment_type==3) {
            let img_dwn_icon = "";
            ...
            success: function (data) {
                ...
                if(response.status==1){
                    ...
                    if(response.type==1){
                        ...
                        // var comment_html = '<li class="wpf_author"><level class="task-author">'+current_user_name+' '+wpf_just_now+'</level><div class="meassage_area_main"><a href="'+response.message+'" download="'+wpf_download_file+'" target="blank"><i class="gg-software-download"></i> '+ wpf_download_file+'</a></div></li>';
