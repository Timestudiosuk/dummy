...
//         }
//
//         // shift the compact mode option with sidebar => v2.1.0
//         if ((typeof jQuery_WPF('body').find('.wpf-compact-launcher').attr('id') !== 'undefined')) {
//             jQuery_WPF('#wpf_launcher .wpf_sidebar_container').parent().removeClass('wpf-compact-launcher-with-sidebar');
//         }
//     } else {
...
//         if ((typeof jQuery_WPF('body').find('.wpf-compact-launcher').attr('id') !== 'undefined')) {
//             setTimeout(function () {
//                 jQuery_WPF('#wpf_launcher .wpf_sidebar_container').parent().addClass('wpf-compact-launcher-with-sidebar');
//             }, 0);
//         }
//     }
// }
...
function expand_bottom_bar(){
	...
	is_expand = '0';
//	jQuery_WPF('body').css({"bottom":"50px", "position" : "relative"});
	
	jQuery_WPF('#wpf_launcher .wpf_launch_buttons').css({"bottom":"5px"});
        jQuery_WPF('#wpf_launcher #wpf_bottom_bar').css({"bottom": "-49px"});
...
//	jQuery_WPF('body').css({"bottom":"50px", "position" : "relative"});
	...
	jQuery_WPF('#wpf_launcher .wpf_launch_buttons').css({"bottom":"-49px"});
...
//if(jQuery_WPF('#wpf_bottom_bar').length > 0) {
//    jQuery_WPF('body').css({"bottom":"50px", "position" : "relative"});
//}
        ...
        jQuery_WPF.ajax({
            ...
            success: function(data){
                ...
                if(data == 1){
                    ...
                    //jQuery_WPF('#wpf_approve_modal').hide();
...
jQuery_WPF('.wpf_responsive_tablet').on('click',function(){
    ...
    jQuery_WPF(".iframe_holder iframe").removeAttr('src');
    jQuery_WPF('.iframe_holder').css('width','798px').css('height','1144px');
    jQuery_WPF('#responsive_modal').show();
    setTimeout(function(){
        jQuery_WPF(".iframe_holder iframe").attr("src",location.href);
    },50);
...
jQuery_WPF('.wpf_responsive_mobile').on('click',function(){
    ...
    jQuery_WPF(".iframe_holder iframe").removeAttr('src');
    jQuery_WPF('.iframe_holder').css('width','405px').css('height','787px');
    jQuery_WPF('#responsive_modal').show();
    setTimeout(function(){
        jQuery_WPF(".iframe_holder iframe").attr("src",location.href);
    },50);
...
function wpf_image_open_new_tab(e){
    ...
    // console.log(jQuery_WPF(e).parent().find('a')[0].getAttribute('href'));
}
...
function search_filter(task_title) {
    ...
    jQuery(active_tab).find('ul > li').each(function(index, element) {
        ...
        if ( jQuery(element).find('.current_page_task_list').text().toLowerCase().includes(task_title.toLowerCase()) ) {
            // jQuery(active_tab).find('div.nothingfound').remove();
    ...
    if ( jQuery(active_tab).find('ul > li').length === count ) {
        // jQuery(active_tab).append(
        //     '<div class="current_page_task nothingfound"><img ' +
        //     'src="/wp-content/plugins/atarim-client-interface-plugin-fm-2.1.1/images//nothingfound.svg"></div>'
        // );
...
/**
 * Return the WP user name
 */
// function get_wp_author_name(wpfb_metas)
// {
//     const wpfb_users_arr = JSON.parse(wpfb_users);

//     for (var key in wpfb_users_arr) {

//         if (wpfb_users_arr.hasOwnProperty(key)) {

//             if ( eval ( wpfb_users_arr[key]['id'] ) === eval( wpfb_metas.task_config_author_id ) ) {

//                 return wpfb_users_arr[key]['first_name'] ?
//                 `${wpfb_users_arr[key]['first_name']} ${wpfb_users_arr[key]['last_name']}` : wpfb_users_arr[key]['displayname'];

//             // } else if(wpfb_users_arr[key]['username']==wpfb_metas.task_config_author_name){

//             //     return wpfb_users_arr[key]['displayname'];
//             } 
//         }
//     }

//     return wpfb_metas.task_config_author_name;
// }
...
function countdownTimeStartBackword(deadline, offset=0) {
    ...
    var x = setInterval(function () {const dateNow = new Date();
        ...
        if (countDownDate < now) {
            // countdownTimeStartForword(deadline);
            // clearInterval(x);
            ...
            seconds = ( (seconds > 0 && minutes===0 && hours===0 && days===0) && (countDownDate < now) ) ? `-${seconds}` : seconds;
        }

        // Output the result in an element with id="demo"
        const timer = `${days}d : ${hours}h : ${minutes}m : ${seconds}s`;
        // console.log(timer);
        jQuery_WPF('#site_milestone #countdown_timer').html(timer);
...
function start_milestone_timer(milestone)
{
    ...
    console.log("TIME:", deadline.getTime(), now.getTime(), deadline.toString(), new Date().toString(), deadline.getTime() > now.getTime());
...
// milestone popover
jQuery_WPF('#site_milestone').on('shown.bs.popover', function () {
    
    const element = jQuery_WPF('#site_milestone');
    ...
    jQuery_WPF.ajax({
        ...
        success: function(data){
           ...
           let html = '';
            ...
            // jQuery_WPF(element).attr('data-content', html);
            // var divElem = jQuery_WPF('#site_milestone');
            // divElem.data('bs.popover').element.dataset.content = html;
            // divElem.data('bs.popover').setContent();

            // jQuery_WPF(element).attr('data-content', html);
            // jQuery_WPF(element).show();

            jQuery_WPF('.milestone_popover .popover-body').html( html );
        }
    });
});
