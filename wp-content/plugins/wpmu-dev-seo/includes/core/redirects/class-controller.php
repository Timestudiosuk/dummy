...
class Controller extends Controllers\Controller {
	...
	public function should_run() {
		...
		return ( ! isset( $adv_tools_options['disable-adv-tools'] ) || ! $adv_tools_options['disable-adv-tools'] ) &&
				Admin_Settings::is_tab_allowed( Settings::TAB_AUTOLINKS ) && Settings::get_setting( 'redirects', true );
	}
	...
	protected function init() {
		...
		add_action( 'wp_ajax_wds_import_redirects', array( $this, 'import_redirects' ) );
		add_action( 'wp_ajax_wds_export_redirects', array( $this, 'export_redirects' ) );
				...
				echo wp_kses_post(
					...
					/* translators: 1, 2: strong tag, 3: post type label, 4: post stats transition action, 5, 6: Link to redirection page, 7: Redirection source url. */
						...
						/* translators: 1: Advanced Tools page url, 2: Redirection source url */
							...
							Admin_Settings::admin_url( Settings::TAB_AUTOLINKS ),
							$transient_data['url']
						),
						'</a>',
						$transient_data['url'],
	...
	private function format_destination( $destination ) {
		...
		if ( empty( $destination['id'] ) ) {
			...
			if ( $post_url ) {
				...
				$destination['type']  = get_post_type_object( get_post_type( $post_id ) )->labels->singular_name;
				$destination['_type'] = get_post_type( $post_id );
			}
		}
