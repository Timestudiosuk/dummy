...
use SmartCrawl\Redirects\Utils;
...
class Assets extends Controller {
	...
	private function register_advanced_tools_scripts() {
		if ( ! $this->is_page( Settings::TAB_AUTOLINKS ) ) {
			return;
		}
		...
		$args = array(
			...
			'non_redirect_types' => Utils::get()->get_non_redirect_types(),
		);

		if ( \SmartCrawl\Redirects\Controller::get()->should_run() ) {
			$args['enabled']            = true;
			$args['redirects']          = \SmartCrawl\Redirects\Controller::get()->get_redirects();
			$args['accepted-csv-types'] = \smartcrawl_csv_mime_types();
			$args['maxmind_license']    = GeoDB::get()->get_license();
			$args['option_name']        = 'wds_autolinks_options';

		} else {
			$args['enabled']        = false;
			$args['settings_nonce'] = wp_create_nonce( 'wds-settings-nonce' );
			$args['referer']        = add_query_arg( 'tab', 'tab_url_redirection', remove_query_arg( '_wp_http_referer' ) );

			$image_url = sprintf( '%s/assets/images/empty-box.svg', SMARTCRAWL_PLUGIN_URL );

			$args['image'] = White_Label::get()->get_wpmudev_hero_image( $image_url );
		}

		if ( (bool) \smartcrawl_get_array_value( $options, 'redirect-attachments' ) ) {
			$args['redirect-attachments'] = (bool) \smartcrawl_get_array_value( $options, 'redirect-attachments' );
		}

		if ( (bool) \smartcrawl_get_array_value( $options, 'redirect-attachments-images_only' ) ) {
			$args['redirect-attachments-images_only'] = (bool) \smartcrawl_get_array_value( $options, 'redirect-attachments-images_only' );
		}

		if ( \smartcrawl_get_array_value( $options, 'redirections-code' ) ) {
			$args['redirections-code'] = \smartcrawl_get_array_value( $options, 'redirections-code' );
		}
