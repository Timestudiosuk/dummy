...
class Env {
	...
	public static function get_current_page_url( $append = array() ) {
		...
		if ( isset( $_GET['set_apikey'] ) ) {
			...
			$searchString = '&api_error=auth&auth_error=google_unlinked';
			$to_base      = $_SERVER['REQUEST_URI'];
			if ( substr( $to_base, -strlen( $searchString ) ) === $searchString ) {
				$to_base = substr( $to_base, 0, -strlen( $searchString ) );
			}
